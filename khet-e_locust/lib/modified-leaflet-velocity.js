...
L.LocustVelocityLayer = (L.Layer ? L.Layer : L.Class).extend({
    options: {
        displayValues: true,
        displayOptions: {
    ...
    _initWindy: function _initWindy(self) {
        ...
        this._windy = new LocustWindy(options); // prepare context global var, start drawing
...
L.locustVelocityLayer = function(options) {
    return new L.LocustVelocityLayer(options);
};
...
var LocustWindy = function LocustWindy(params) {
    ...
    var MAX_PARTICLES = params.maxParticles || 2;
    ...
    var createField = function createField(columns, bounds, callback) {
        ...
        field.randomize = function(o) {
            ...
            do {
                const locustPoint = LOCUST_POINTS[Math.floor(Math.random() * LOCUST_POINTS.length)];
                const pixelCoords = project(locustPoint[0], locustPoint[1]);
                // console.log(pixelCoords);

                // if (pixelCoords[0] < bounds.x || pixelCoords[1] < bounds.y || pixelCoords[0] > bounds.x + bounds.width || pixelCoords[1] > bounds.y + bounds.height) {
                //     // not on screen
                //     // console.log('failed');
                //     return undefined;
                // }
                // TODO: make this pick the coordinates that I want it to
                x = pixelCoords[0];
                y = pixelCoords[1];
                // x = Math.round(Math.floor(Math.random() * bounds.width) + bounds.x);
                // y = Math.round(Math.floor(Math.random() * bounds.height) + bounds.y);
                // while loop checks whether wind data exists for specific point
    ...
    var animate = function animate(bounds, field) {
        ...
        var particleCount = MAX_PARTICLES;

        var fadeFillStyle = "rgba(0, 0, 0, ".concat(OPACITY, ")");
        var particles = [];

        for (var i = 0; i < particleCount; i++) {
            // Make sure point is returned
            var point = field.randomize({
            ...
            buckets.forEach(function(bucket, i) {
                if (bucket.length > 0) {
                    ...
                    bucket.forEach(function(particle) {
                        ...
                        // g.beginPath();
                        // g.arc(particle.x, particle.y, (PARTICLE_LINE_WIDTH / 2) * (particle.age / MAX_PARTICLE_AGE), 0, 2 * Math.PI, false);
